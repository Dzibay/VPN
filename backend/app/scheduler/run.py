...
* **periodic** — сбор трафика Xray (после батча: лимит, sync, ``notify_traffic_*``), ежедневный sync
  клиентов Xray (RQ) и ``notify_sub_expire`` (00:05 МСК), sync нагрузки из Prometheus, фоновый TCP-опрос узлов.
* **telegram_notify** — раз в сутки ``notify_sub_expire_3d/1d/0d`` и ``notify_sub_expired_7d`` (12:00 МСК);
  периодически (~5 мин) ``notify_reg_1h_has_traffic`` / ``notify_reg_1h_no_traffic`` после регистрации.
"""
