"""
Ежедневная постановка в RQ полной синхронизации клиентов Xray на всех узлах.

Список UUID на серверах собирается из пользователей с активной подпиской
(см. app.services.xray_clients.active_subscription_users), поэтому после истечения
срока клиент пропадает с прокси после очередного sync — отдельное поле в БД не нужно.
"""
...
from app.services.user_provision import ensure_sync_xray_clients_all_servers_enqueued

log = logging.getLogger("app.subscription_daily_xray_sync")
...
def run_daily_xray_clients_sync_enqueue() -> None:
    """Поставить в очередь sync inbound (идемпотентно, см. coalesce по job_id в user_provision)."""
    try:
