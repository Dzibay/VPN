...
const props = defineProps({
  ...
  /** @type {Array<{ id: number, name: string, color?: string, link_ids?: number[] }>} */
  items: {
    type: Array,
    default: () => [],
  },
  ...
  placeholder: {
    type: String,
    default: 'Поиск группы по названию',
  },
  minQueryLength: {
    type: Number,
    default: 1,
...
function syncTextFromValue() {
  ...
  const row = props.items.find((g) => Number(g.id) === id)
...
function formatRowLabel(row) {
  const count = row.link_ids?.length ?? 0
  return `${row.name} (${count} ток.)`
}
    ...
    <div v-show="open" class="suggest-panel" role="listbox">
      ...
      <template v-else>
        <button
          ...
          @click="pickRow(row)"
        >
          <span class="suggest-option-row">
            <span
              class="suggest-option-dot"
              :style="{ background: row.color || '#58d68d' }"
              aria-hidden="true"
            />
            <span class="suggest-option-id">{{ row.name }}</span>
          </span>
          <span class="suggest-option-meta">
            {{ row.link_ids?.length ?? 0 }} токенов · #{{ row.id }}
          </span>
        </button>
      </template>
    </div>
